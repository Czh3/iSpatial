...
NA_merFISH1 = stabilize_expr(NA_merFISH)
trace("stabilize_expr", edit=TRUE)

# infer spatial transcriptome
MSN_merFISH_iSpatial = iSpatial::iSpatial(NA_merFISH, NA_scRNA)
...
SVGs_infer = spatial_variable_genes(MSN_merFISH_iSpatial, "enhanced", n.core = 10)
SVGs_infer_sig = SVGs_infer[SVGs_infer$p.value < 0.001, ]

g = ggplot(SVGs_infer, aes(pct.expr, -log10(p.value), gene = gene, color = -log10(p.value))) +
  geom_point()+
  theme_classic()
plotly::ggplotly(g)
...
# genes in cluster 6
names(gene_cluster[gene_cluster==6])

# plot
spatial_signature_plot(MSN_merFISH_iSpatial, 
                       gene.set = names(gene_cluster[gene_cluster==6]),
                       set.name = "Class 6")
